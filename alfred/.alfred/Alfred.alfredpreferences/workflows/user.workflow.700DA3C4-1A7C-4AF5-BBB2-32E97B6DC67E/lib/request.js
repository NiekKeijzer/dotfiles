...
exports.request = function (query) {
  let opts = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
  let url = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 'https://search.elastic.co/suggest';

  if (query.q === null || query.q.trim().length == 0) {
    return Promise.resolve([]);
  }
  return alfy.fetch(url, Object.assign(opts, { query: query })).then(res => {
    if (res.hits.length == 0) {
      return Promise.resolve([{ title: 'No results found', icon: { path: rootDir + '/icon.png' } }]);
    }
    const items = res.hits.map(hit => {
      var product = _.find(Object.keys(icons), name => {
        return hit.breadcrumbs.toLowerCase().includes(name);
      });
